function setupMap()
{
    ...
    var leafletCdn = "https://cdn.jsdelivr.net/npm/leaflet@1.9.3/dist/";
    var esriLeafletCdn = "https://cdn.jsdelivr.net/npm/esri-leaflet@3.0.8/dist/";
    var clusterLeafletCdn = "https://cdn.jsdelivr.net/npm/leaflet.markercluster@1.5.3/dist/";
   ...
   var defaultMapElStyle = "margin-top:15px; margin-bottom:15px";
...
// that is visible on the page
    ...
    function _setPopupContent(item){
        var potentialPopup = "";
        if('popupContent' in item){
                potentialPopup += item.popupContent;}
        if('href' in item){
            potentialPopup += '<a href="' + item.href + '">' + 
                '<img src="' + newWindowImgSrcBase64 + '"></img></a>';}
        return potentialPopup;
    }

    function _setIconContent(item){
        var hash = {iconUrl: item.icon.iconUrl};
        if (item.icon.iconRetinaUrl) {hash.iconRetinaUrl = item.icon.iconRetinaUrl}
        if (item.icon.iconSize) {hash.iconSize = item.icon.iconSize}
        if (item.icon.iconAnchor) {hash.iconAnchor = item.icon.iconAnchor}
        if (item.icon.popupAnchor) {hash.popupAnchor = item.icon.popupAnchor}
        if (item.icon.shadowUrl) {hash.shadowUrl = item.icon.shadowUrl}
        if (item.icon.shadowRetinaUrl) {hash.shadowRetinaUrl = item.icon.shadowRetinaUrl}
        if (item.icon.shadowSize) {hash.shadowSize = item.icon.shadowSize}
        if (item.icon.shadowAnchor) {hash.shadowAnchor = item.icon.shadowAnchor}
        return L.icon(hash);
    }
    ...
    function _addMarkerToMap(leafletItem, map){
        ...
        var result;
        if (m.icon === undefined || m.icon.iconUrl === undefined) {
            result = L.marker([m.latitude, m.longitude]).addTo(map);
        }else {
            var customIcon = _setIconContent(m);
            result = L.marker([m.latitude, m.longitude], {icon: customIcon}).addTo(map);
        }
        var potentialPopup = _setPopupContent(m);
        if(potentialPopup){
    ...
    function _addMarkerToCluster(leafletItem, clusterGroup){
        var m = leafletItem.value;
        var result;
        if (m.icon === undefined || m.icon.iconUrl === undefined) {
            result = L.marker([m.latitude, m.longitude]);
        }else {
            var customIcon = _setIconContent(m);
            result = L.marker([m.latitude, m.longitude], {icon: customIcon});
        }
    ...
    function _processLeafletItem(leafletItem, map, clusterGroup){
        switch(leafletItem.type){
            ...
            case "LeafletGeoJSON":
                ...
                break;
            case "LeafletClusterMarker":
                _addMarkerToCluster(leafletItem, clusterGroup);
                break;
            case undefined:
                break; 
            default:
    ...
    function createMap(){
        ...
        var clusterGroup = L.markerClusterGroup({
			...
			iconCreateFunction: function (cluster) {
				return L.divIcon({ html: "<p>"+cluster.getChildCount()+"</p>", className: 'mycluster', iconSize: L.point(40, 40) });
			}
		});
        ...
        for(var i=0; i<blockLeafletItems.length; i++){
            ...
            _processLeafletItem(leafletItem, map, clusterGroup);
        }
...
// When ready, call createMap() to create the map on the page
    ...
    var clusterLeafletJsId = "cluster-leaflet-js-head";
    var leafletProvidersJsId = "leaflet-providers-js-head";
    var clusterLeafletCssId = "cluster-leaflet-css-head";
    var clusterLeafletDefCssId = "cluster-leaflet-def-css-head";
    ...
    leafletCssEl.href = leafletCdn + "leaflet.min.css";
    if(!document.getElementById(leafletCssEl.id)){
        document.head.appendChild(leafletCssEl);
    }

    // Add the cluster-leaflet CSS first, don't worry about when it loads
    var clusterLeafletCssEl = document.createElement("link");
    clusterLeafletCssEl.id = clusterLeafletCssId;
    clusterLeafletCssEl.rel = "stylesheet";
    clusterLeafletCssEl.href = clusterLeafletCdn + "MarkerCluster.css";
    if(!document.getElementById(clusterLeafletCssEl.id)){
        document.head.appendChild(clusterLeafletCssEl);
    }

    // Add the cluster-leaflet CSS first, don't worry about when it loads
    var clusterLeafletDefCssEl = document.createElement("link");
    clusterLeafletDefCssEl.id = clusterLeafletDefCssId;
    clusterLeafletDefCssEl.rel = "stylesheet";
    clusterLeafletDefCssEl.href = clusterLeafletCdn + "MarkerCluster.Default.css";
    if(!document.getElementById(clusterLeafletDefCssEl.id)){
        document.head.appendChild(clusterLeafletDefCssEl);
    }
    ...
    leafletJsEl.onload = () => {
        ...
        esriEl.onload = () => {
            //After loaded, load the cluster-leaflet.js code, wait for it to load
            var clusterEl = document.createElement("script");
            clusterEl.id = clusterLeafletJsId;
            clusterEl.onload = () => {
            ...
            clusterEl.src = clusterLeafletCdn + "leaflet.markercluster.js";
            addToHeadIfNotLoaded(clusterEl);
        };
        esriEl.src = esriLeafletCdn + "esri-leaflet.min.js";
        addToHeadIfNotLoaded(esriEl);
    };
    leafletJsEl.src = leafletCdn + "leaflet.min.js";
    addToHeadIfNotLoaded(leafletJsEl);
}
function checkConsentCookie() {
    var site_config = %{site_config};
    if(site_config === null)
        return true; // No consent configured
    let name = site_config["consentCookie"]
    let acceptance = site_config['consentAcceptance']
    var nameEQ = name + "=";
    ...
    for(var i=0;i < ca.length;i++) {
        ...
        if (c.indexOf(nameEQ) == 0)
            return c.substring(nameEQ.length,c.length) == acceptance;
    }
    return false;
}
